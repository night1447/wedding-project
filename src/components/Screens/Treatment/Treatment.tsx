...
export const Treatment: FC<TreatmentProps> = ({people}) => {
    return (
        <WrapperSection wrapperClass={styles.wrapper} sectionClass={styles.treatment} id={'treatment'}>
            ...
            <p className={styles.description}>
                {!people.isFamily ?
                    `Очень ждем ${people.people.length > 1 ? 'вас' : ''} на нашей СВАДЬБЕ и не представляем этого праздника без ${people.people.length === 1 ? 'тебя' : 'вас'}, ${people.people.length === 1 ? 'самого близкого нам человека' : 'самых близких нам людей'} ❤️` :
